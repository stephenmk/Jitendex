...
//
// This file, ConsecutiveKanjiAlgorithm.cs, is part of Jitendex.
//
...
internal sealed class ConsecutiveKanjiAlgorithm : CharacterAlgorithm
{
    public override ImmutableArray<ImmutableArray<SolutionPart>> Solve(in TextSlice textSlice, in ReadingState readingState)
    {
        ...
        foreach (var rune in textSlice.Runes)
        {
            if (!rune.IsKanji())
            {
                return [];
        ...
        if (RegexReading(textSlice, readingState) is not string regexReading)
        {
            return [];
        }
